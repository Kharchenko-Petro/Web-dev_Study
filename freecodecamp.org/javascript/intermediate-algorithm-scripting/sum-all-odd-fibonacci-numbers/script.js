...
// the sum of all odd Fibonacci numbers 
...
function sumFibs(num) {
  const arrOfFibsNum = [1];
  let currentFibsNum = 1;
  let counter = 0;

  while (currentFibsNum <= num) {
    arrOfFibsNum.push(currentFibsNum);
    currentFibsNum += arrOfFibsNum[counter];
    counter++;
  }

  const sumOfFibs = arrOfFibsNum.reduce( (sum, item) => {
    if (item % 2 !== 0) {
      return sum + item;
    }
